 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `PAModel` protocol for custom methods.


 Sample Code:

     ********************** json convertor *********************
     @interfaceClass PAAuthor : NSObject
     @property (nonatomic, strong) NSString *name;
     @property (nonatomic, assign) NSDate *birthday;
     @end
     @implementation PAAuthor
     @end

     @interfaceClass PABook : NSObject
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) NSUInteger pages;
     @property (nonatomic, strong) PAAuthor *author;
     @end
     @implementation PABook
     @end

     int main() {
         // create model from json
         PABook *book = [PABook pa_modelWithJSON:@"{\"name\": \"Harry Potter\", \"pages\": 256, \"author\":
 {\"name\": \"J.K.Rowling\", \"birthday\": \"1965-07-31\" }}"];
     ...
     ********************** Coding/Copying/hash/equal *********************
     @interfaceClass PAShadow :NSObject <NSCoding, NSCopying>
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) CGSize size;
     @end

     @implementation PAShadow
 ...
 */
@interface NSDictionary (PAModel)
...
/**
 ...
 There's no need to add '<PAModel>' to your class header.
 */
@protocol PAModel <NSObject>
@optional
...
/**
 ...
 object will be add to the array/set/dictionary.
  ...
  Example:
        @interfaceClass PAShadow, PABorder, PAAttachment;

        @interfaceClass PAAttributes
        @property NSString *name;
        ...
        @implementation PAAttributes
        + (NSDictionary *)modelContainerPropertyGenericClass {
            return @{@"shadows" : [PAShadow class],
                     @"borders" : PABorder.class,
                     @"attachments" : @"PAAttachment" };
        }
        @end
...
/**
 ...
 Example:
        @interfaceClass PACircle, PARectangle, PALine;

        @implementation PAShape

        + (Class)modelCustomClassForDictionary:(NSDictionary*)dictionary {
            if (dictionary[@"radius"] != nil) {
                return [PACircle class];
            } else if (dictionary[@"width"] != nil) {
                return [PARectangle class];
